    ...
    <Menu isOpen={isOpen} onStateChange={state => setIsOpen(state.isOpen)} right disableAutoFocus>
      {isAdmin && (
        <>
          ...
          <SidebarLink route={ROUTES.settings} handleClick={closeMenu} />
        </>
      )}
      ...
      {isAdmin && (
        <a className="bm-item" href="" onClick={() => auth.logout()} >
          Logout
        </a>
      )}
    </Menu>
...
const SettingsPageWithAuthentication = withAuthentication(SettingsPage);
    ...
    <main className="app-content backoffice">
      <div className="container">
        <Switch>
          ...
          <Route exact path={ROUTES.addressManagement.path} render={() => <AddressManagementPageWithAuthentication/>} />
          ...
          <Route  exact path={ROUTES.settings.path} render={() => <SettingsPageWithAuthentication/>} />
